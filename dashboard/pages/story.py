...
def _build_story_context(date_from: date | None = None, date_to: date | None = None) -> dict:
    ...
    if not stats.empty:
        ...
        if "FirstEliminated" in stats.columns and "first_knocked_out" not in tr_full:
            _fko_rows = stats[pd.to_numeric(stats["FirstEliminated"], errors="coerce").fillna(0).astype(int) == 1]
            if not _fko_rows.empty:
                tr_full["first_knocked_out"] = str(_fko_rows.iloc[0]["Team"])
    ...
    _PRED_CATEGORIES = [
        ...
        ("First Knocked Out","first_knocked_out","first_ko_bonus", 20),
    ]
